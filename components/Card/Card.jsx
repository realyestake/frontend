...
export function CardDefault({ name, location, details, price, isNew, isLiked, pictures, _id }) {
  console.log("id", _id, name, location, details, price, isNew, isLiked, pictures)
  ...
  const handleChangeColour = async () => {
    ...
    console.log('id', _id);
    const response = await axios.put(`${BACKEND_URL}/api/properties/favourite/${_id}`, {
    }, {
      headers: {
  ...
  return (
    <Card className="mt-6  rounded-sm bg-[#220526]" key={_id}>
      ...
      <CardHeader floated={false}  className="relative m-0 rounded-sm">
        <img
          src={pictures!=="images[0]" ? pictures: "https://images.unsplash.com/photo-1560518883-ce09059eeffa?q=80&w=1000&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8cHJvcGVydHl8ZW58MHx8MHx8fDA%3D"}
          alt="card-image"
      ...
      <CardBody>
        <Typography variant="h6" color="blue-gray" className="mb-2 xl  text-white capitalize">
          {name}
        </Typography>
        ...
        <Typography variant="small" color="blue-gray" className="mb-2 xl  text-white/70">
          ...
          {details && details.split(' ').slice(0, 10).join(' ') + '...'}
        </Typography>
      </CardBody>
