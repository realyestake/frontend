...
import { useEffect, useState } from "react";
  ...
  export function UserCard({custType , ...data}) {

    const { _id, firstName, lastName, email,  bio, profilePicture,  phoneIsVisible, addressIsvisible
    } = data;
    ...
    //       "http://localhost:3000/api/customers/profile/"+_id,
    //       {
    //         headers: {
    ...
    return (
      <Card className="mt-6 w-full rounded-md border-2 border-white/20 bg-[#110213] text-white px-[1rem]">
        <CardBody className="flex flex-col pl-[1rem] pr-[1rem] gap-[1.5rem]">
          <div className="flex flex-row gap-[1.5rem]">
            ...
            <div className="flex flex-col justify-center text-start">
              <Typography color="gray" className="lg:text-[24px] text-[18px] text-white font-bold">{firstName}{" "}{lastName}</Typography>
          ...
          <div className="flex flex-col gap-2 text-start justify-center">
            ...
            {custType !== "individual" ? (
              ...
              <div className="flex flex-row items-center">
                ...
                <Typography color="gray" className="text-[12px] text-white/50 pl-2">
                  New Company
                </Typography>
              </div>
              <div className="flex flex-row items-center">
        ...
        <CardFooter className="pt-0 w-full">
          <Link href={`/admin/user-profile/${_id}`} className="inline-block w-full justify-center my-0 ">
            <Button size="sm" className="rounded-full border border-white w-full text-white bg-[#110213]">
              <Typography color="white" className="text-sm">View Profile</Typography>
            </Button>
          </Link>
        </CardFooter>
      </Card>
